...
typedef struct {
    uint8_t     fileMagic[4];       /* MLVI: Magic Lantern Video file header */
...
typedef struct {
    uint8_t     blockType[4];       /* VIDF: this block contains one frame of video data */
...
typedef struct {
    uint8_t     blockType[4];       /* AUDF: this block contains audio data */
    uint32_t    blockSize;          /* total frame size */
    uint64_t    timestamp;          /* hardware counter timestamp for this frame (relative to recording start) */
...
typedef struct {
    uint8_t     blockType[4];       /* RAWI: when videoClass is RAW, this block will contain detailed format information */
    uint32_t    blockSize;          /* total frame size */
    uint64_t    timestamp;          /* hardware counter timestamp for this frame (relative to recording start) */
...
typedef struct {
    uint8_t     blockType[4];       /* WAVI: when audioClass is WAV, this block contains format details  compatible to RIFF */
    uint32_t    blockSize;          /* total frame size */
    uint64_t    timestamp;          /* hardware counter timestamp for this frame (relative to recording start) */
...
typedef struct {
    uint8_t     blockType[4];       /* EXPO: exposure information (ISO, shutter) */
    uint32_t    blockSize;          /* total frame size */
    uint64_t    timestamp;          /* hardware counter timestamp for this frame (relative to recording start) */
...
typedef struct {
    uint8_t     blockType[4];       /* LENS: lens information (aperture, focal len, focus distance, lens name...) */
    uint32_t    blockSize;          /* total frame size */
    uint64_t    timestamp;          /* hardware counter timestamp for this frame (relative to recording start) */
...
typedef struct {
    uint8_t     blockType[4];       /* IDNT: camera identification (model, serial number) */
    uint32_t    blockSize;          /* total frame size */
    uint64_t    timestamp;          /* hardware counter timestamp for this frame (relative to recording start) */
...
typedef struct {
    uint8_t     blockType[4];       /* XREF: can be added in post processing when out of order data is present */
...
typedef struct {
    uint8_t     blockType[4];       /* INFO: user definable info string. take number, location, etc. */
    uint32_t    blockSize;
    uint64_t    timestamp;
...
typedef struct {
    uint8_t     blockType[4];       /* DISO: Dual-ISO information */
    uint32_t    blockSize;
    uint64_t    timestamp;
...
typedef struct {
    uint8_t     blockType[4];       /* MARK: markers set by user while recording */
    uint32_t    blockSize;
    uint64_t    timestamp;
...
typedef struct {
    uint8_t     blockType[4];       /* STYL: picture style information */
    uint32_t    blockSize;
    uint64_t    timestamp;
    uint32_t    picStyleId;
    int32_t     contrast;
...
typedef struct {
    uint8_t     blockType[4];       /* ELVL: picture style information */
    uint32_t    blockSize;
    uint64_t    timestamp;
...
typedef struct {
    uint8_t     blockType[4];       /* WBAL: White balance info */
    uint32_t    blockSize;
    uint64_t    timestamp;
